import a1_excercise as solution
...
''')
"""
StrinIO makes a file out of a string. For whatever reason?
"""

def test_no_places(monkeypatch): # what is monkeypatch? Object from Pytest?
    # Prepare setattr.
    # Our function collect_places() waits for user_input
    # That will be handled with the data from empty_place_inputs
    # empt_place_inputs
...
def test_one_place(monkeypatch):
    """
    collect places is waiting for input.
    Here we simulate with monkeypatch.setattr the input of one_place_input
    one_place_input = StringIO('London, England\n\n')
    """
...
def test_sorting_cities(monkeypatch, capsys):
    ...
    solution.collect_places()
    # Reading from the standard out and error files.
    # captured out captures the output of the function collect_places()
    captured_out, captured_err = capsys.readouterr()
    """
    Values: 
    captured_err = str("") --> means captured_error??
    captured_out = str('Tell me where you went: Tell me where you went: Tell me where you went: Tell me where you went: ')
    """
